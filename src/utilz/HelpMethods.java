...
public class HelpMethods {
	...
	public static ArrayList<Enemy> getEnemies(BufferedImage img, Play play) {
		...
		for (int j = 0; j < img.getHeight(); j++) {
			for (int i = 0; i < img.getWidth(); i++) {
				...
				if (value == CRABBY) {
					enemies.add(new Crabby((int) (i * GamePanel.TILE_SIZE), (int) (j * GamePanel.TILE_SIZE), play));
				}
			}
		}
		return enemies;
	...
	public static ArrayList<Potion> getPotionLst(BufferedImage img) {
		...
		for (int j = 0; j < img.getHeight(); j++) {
			for (int i = 0; i < img.getWidth(); i++) {
				...
				if (value == RED_POTION || value == BLUE_POTION) {
					potions.add(new Potion((int) (i * GamePanel.TILE_SIZE), (int) (j * GamePanel.TILE_SIZE), value));
				}
			}
		}
		return potions;
	...
	public static ArrayList<GameContainer> getContainerLst(BufferedImage img) {
		...
		for (int j = 0; j < img.getHeight(); j++) {
			for (int i = 0; i < img.getWidth(); i++) {
				...
				if (value == BOX || value == BARREL) {
					containers.add(
							new GameContainer((int) (i * GamePanel.TILE_SIZE), (int) (j * GamePanel.TILE_SIZE), value));
				}
			}
		}
		return containers;
	...
	public static ArrayList<Spike> getSpikeLst(BufferedImage img) {
		...
		for (int j = 0; j < img.getHeight(); j++) {
			for (int i = 0; i < img.getWidth(); i++) {
				...
				int value = color.getBlue();
				if (value == SPIKE) {
					spikes.add(new Spike((int) (i * GamePanel.TILE_SIZE), (int) (j * GamePanel.TILE_SIZE), SPIKE));
				}
			}
		}
		return spikes;
	...
	public static ArrayList<Canon> getCanonLst(BufferedImage img) {
		ArrayList<Canon> canons = new ArrayList<>();
		for (int j = 0; j < img.getHeight(); j++) {
			for (int i = 0; i < img.getWidth(); i++) {
				Color color = new Color(img.getRGB(i, j));
				int value = color.getBlue();
				if (value == CANON_LEFT || value == CANON_RIGHT) {
					canons.add(new Canon((int) (i * GamePanel.TILE_SIZE), (int) (j * GamePanel.TILE_SIZE), value));
				}
			}
		}
		return canons;
	...
	public static Point getPlayerSpawn(BufferedImage img) {
		for (int j = 0; j < img.getHeight(); j++) {
			for (int i = 0; i < img.getWidth(); i++) {
				...
				int value = color.getGreen();
				if (value == 100) {
					return new Point(i * GamePanel.TILE_SIZE, j * GamePanel.TILE_SIZE);
				}
			}
		}
		return new Point(1 * GamePanel.TILE_SIZE, 1 * GamePanel.TILE_SIZE);
	}
}
