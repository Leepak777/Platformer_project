...
public class Rain {
	...
	private Point2D.Float getRndPos() {
		return new Point2D.Float((int) getNewX(0), rand.nextInt(GamePanel.GAME_HEIGHT));
	}

	public void update(int xLvlOffset) {
		for (Point2D.Float p : drops) {
			p.y += rainSpeed;
			if (p.y >= GamePanel.GAME_HEIGHT) {
				p.y = -20;
	...
	private float getNewX(int xLvlOffset) {
		float value = (-GamePanel.GAME_WIDTH) + rand.nextInt((int) (GamePanel.GAME_WIDTH * 3f)) + xLvlOffset;
		return value;
	}
